// Package crudcrud is a sample connector that wraps the crudcrud.com
// REST sandbox — a free, throwaway JSON store useful for demos and
// integration smoke tests. One row in crudcrud is a JSON document under
// an arbitrary {resource} collection; documents are addressed by an
// auto-generated _id.
//
...
//
// Operation handlers stay readable by delegating: parse + validate
// inputs through service.go, then call repo.go for the upstream request.
package crudcrud
...
// Key is the connector definition slug. Surfaced as the prefix of every
// MCP tool name (`crudcrud__<op>__<instance_label>`).
const Key = "crudcrud"
...
// Operations returns the LLM-callable actions exposed by this
// connector. Order is stable so MCP tools/list output reads
