...
**Versi original (2026-05-10):** Tiga bug independen kombinasi bikin gate keliatan rusak di sesi nyata,
...
## Fix 2026-05-16: Gate bypass — socket & path

Tiga celah bypass yang ditemukan setelah audit menyeluruh gate system:

### #1 — GateLoader tidak cek apakah socket benar-benar running

**File:** `internal/pkg/api/server.go`

**Masalah:** `GateLoader` (closure yang di-call tiap spawn) hanya cek
`gate_enabled` config + `resolvedGateBin != ""`. Kalau `approvalMgr.Start()`
gagal (socket bind error — stale file, permission, port conflict), `gateStatus.Enabled`
...
`approvalMgr.Start()` sukses. GateLoader cek flag ini:

```go
var gateSocketOK bool
agentsFactory.GateLoader = func() *agentpool.GateConfig {
    if !gateSocketOK { return nil }   // ← guard baru
    ...
}
// ... lalu di blok else setelah Start() berhasil:
gateSocketOK = true
```

Kalau socket gagal start → `gateSocketOK = false` → GateLoader return `nil`
→ hooks tidak ditulis → spawner tahu gate tidak aktif → tidak ada fail-open
yang tersembunyi.
...
```
Komentar lama *"safe (CWD = workspace)"* salah asumsi — path dengan `..`
bisa escape workspace walau CWD correct.

**Fix:** Resolve path relatif terhadap `in.CWD` sebelum scope check:
...
if !strings.HasPrefix(path, "/") && !filepath.IsAbs(path) {
    ...
    // path sekarang absolut → lanjut ke scope check normal
}
```

`filepath.Clean` resolve semua `..` sebelum `PathWithinScope` dipanggil,
jadi traversal escape tertangkap konsisten (sama seperti Bash tool yang
sudah punya `argsWithinScope` + `filepath.Join(scope, a)`).

### #3 — `splitCommand` pecah quoted path ber-spasi jadi token salah

**File:** `internal/agents/gate/rule.go` — fungsi `splitCommand`

**Masalah:** Implementasi lama pakai `strings.Fields` + strip quote per-token.
Path dengan spasi dalam quotes dipecah salah:
```
cat "/workspace/my file.txt"
→ fields: ["cat", '"/workspace/my', 'file.txt"']
→ setelah strip: ["cat", "/workspace/my", "file.txt"]
→ scope check cek "/workspace/my" (bukan path asli)
```
Ini scope check false positive — yang dicek bukan path yang sebenarnya diakses.

**Fix:** Tokenizer sederhana yang hormati quote pair (`"..."` dan `'...'`):

```go
func splitCommand(cmd string) ([]string, error) {
    // Track inDouble / inSingle state
    // Whitespace di dalam quotes → bagian token, bukan separator
    // Unclosed quote → error
}
```

Sekarang `cat "/workspace/my file.txt"` → `["cat", "/workspace/my file.txt"]`
→ scope check cek path yang benar.

### Touch point code (fix 2026-05-16)

- [`internal/pkg/api/server.go`](../pkg/api/server.go) — `gateSocketOK` flag + guard di `GateLoader`
- [`cmd/gate/main.go`](../../cmd/gate/main.go) — `runPathGate`: resolve relative path vs CWD
