...
| Phase 6 — Polish | `[ ]` | — |
| Phase 7 — Mid-session Gate Approval + AskUser | `[~]` | Stages 1–7 code-complete (368 unit tests hijau across 84 packages). Whitelist-only gate diupgrade ke interactive approval via Unix socket (gate ↔ daemon) + web UI modal 4-mode (`approve_once`/`approve_session`/`approve_always`/`block`) + Approved-commands panel dgn Revoke. AskUser MCP tool wired (agent panggil → SSE → web card → POST /answer). Sisa: real-claude smoke test (S5.8 + S6.6) + optional doc polish (S7.5/S7.6). Source: [command-gate-architecture.md](./command-gate-architecture.md). |
...
- [ ] **7.8** Smoke test manual: claude jalanin command unlisted → modal muncul → klik tiap mode → behavior konsisten (once tetap muncul, session auto setelah ke-2, always persist after restart). ask_user → card muncul → user pilih → agent terima jawaban. **Belum dijalankan** — manual + butuh real claude binary.
